defmodule CollaborationWeb.CommentControllerTest do
  ...
  def fixture(:comment) do
    {:ok, comment} = Contributions.create_comment(@create_attrs)
    comment
  end
  ...
  describe "edit comment" do
    ...
    test "renders form for editing chosen comment", %{
      conn: conn,
      comment: comment
    } do
  ...
  describe "update comment" do
    ...
    test "redirects when data is valid", %{conn: conn, comment: comment} do
      conn =
        put(conn, comment_path(conn, :update, comment), comment: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, comment: comment} do
      conn =
        put(conn, comment_path(conn, :update, comment), comment: @invalid_attrs)
